...
export default function App() {
  ...
  if (isOp && recTier) {
    builderRate = RATES.builderOp;
    builderHrs = recTier.builderHrs * numBuilders;
    connectorHrs = recTier.connector ? 10 : 0;
  } else if (isSt && recTier) {
    builderRate = RATES.builderSt;
    builderHrs = recTier.builderHrs;
    connectorHrs = recTier.connectorHrs;
    amplifierHrs = recTier.amplifierHrs;
  } else if (isIdeation) {
    builderRate = RATES.builderSt;
    builderHrs = duration * 10;
    connectorHrs = needsConnector ? duration * 2 : 0;
    amplifierHrs = ["strategicGuidance","championing"].some(p=>scores[p]==="M"||scores[p]==="H") ? Math.ceil(duration*1.5) : 0;
  } else if (isExecution) {
    builderRate = RATES.builderEx;
    builderHrs = exBuilderHrs;
    connectorHrs = exConnectorHrs;
    amplifierHrs = showAmplifier ? exAmplifierHrs : 0;
  }
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      if (p.exAmplifierHrs && p.exAmplifierHrs > 0) {
        setExAmplifierHrs(p.exAmplifierHrs);
              ...
              <div className="grid grid-cols-3 text-xs">
                ...
                {[["timebound","Time-Bound"],["ongoing","Ongoing"]].map(([t, tLabel]) => [
                  ...
                  ...["defined","flexible"].map((s) => {
                    ...
                    return <button key={t+s} onClick={() => { setScope(s); setTime(t); }}
            ...
            {isOp && (
              <div className="mt-4 pt-4" style={{ borderTop: `1px solid ${B.border2}` }}>
                ...
                <div className="flex gap-2">{[1,2,3,4].map(n => (
                  <button key={n} onClick={() => setNumBuilders(n)} className="flex-1 py-2 rounded-lg text-sm font-bold transition-all" style={selBtn(numBuilders===n, B.gold)}>{n}</button>
                ))}</div>
              </div>
            )}
            {isIdeation && (
